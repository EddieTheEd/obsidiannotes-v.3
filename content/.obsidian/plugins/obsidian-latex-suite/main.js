  ...
  display() {
    ...
    reset.setIcon("switch").setTooltip("Reset to default snippets").onClick(() => __async(this, null, function* () {
      new ConfirmationModal(this.plugin.app, "Are you sure? This will delete any custom snippets you have written.", (button) => button.setButtonText("Reset to default snippets").setWarning(), () => __async(this, null, function* () {
        this.snippetsEditor.setState(import_state6.EditorState.create({ doc: DEFAULT_SNIPPETS, extensions }));
    ...
    remove.setIcon("trash").setTooltip("Remove all snippets").onClick(() => __async(this, null, function* () {
      new ConfirmationModal(this.plugin.app, "Are you sure? This will delete any custom snippets you have written.", (button) => button.setButtonText("Remove all snippets").setWarning(), () => __async(this, null, function* () {
        ...
        this.snippetsEditor.setState(import_state6.EditorState.create({ doc: value, extensions }));
        updateValidityIndicator(true);
        this.plugin.setSnippets(value);
        this.plugin.settings.snippets = value;
        yield this.plugin.saveSettings();
    ...
    const debouncedSetSnippets = (0, import_obsidian4.debounce)(() => __async(this, null, function* () {
      try {
        yield this.plugin.setSnippetsFromFile(this.plugin.settings.snippetsFileLocation);
        new import_obsidian4.Notice("Successfully loaded snippets from " + this.plugin.settings.snippetsFileLocation + ".", 5e3);
      } catch (e) {
      }
    }), 1e3, true);
    let snippetsFileLocEl;
    new import_obsidian4.Setting(containerEl).setName("Load snippets from file").setDesc("Whether to load snippets from a specified file, instead of from the plugin settings.").addToggle((toggle) => toggle.setValue(this.plugin.settings.loadSnippetsFromFile).onChange((value) => __async(this, null, function* () {
    ...
    const snippetsFileLoc = new import_obsidian4.Setting(containerEl).setName("Snippets file location").setDesc("The file to load snippets from.");
    let inputEl;
    ...
    new import_obsidian4.Setting(containerEl).setName("Enabled").setDesc("When inside inline math, show a popup preview window of the rendered math.").addToggle((toggle) => toggle.setValue(this.plugin.settings.inlineMathPreviewEnabled).onChange((value) => __async(this, null, function* () {
...
function createSnippetsEditor(content, extensions) {
  const view = new import_view8.EditorView({
    state: import_state6.EditorState.create({ doc: content, extensions })
  });
  return view;
}
...
var addMark = import_state7.StateEffect.define();
var removeMark = import_state7.StateEffect.define();
var clearMarks = import_state7.StateEffect.define();
var removeMarkBySpecAttribute = import_state7.StateEffect.define();
...
var markerStateField = import_state7.StateField.define({
  create() {
    return import_view9.Decoration.none;
  },
  update(value, tr) {
  ...
  provide: (f) => import_view9.EditorView.decorations.from(f)
});
...
var SnippetManager = class {
  ...
  expandSnippets(view) {
    ...
    view.dispatch({
      changes: keyPresses,
      annotations: import_commands2.isolateHistory.of("full")
    });
    ...
    const changeSet = import_state8.ChangeSet.of(changes, view.state.doc.length);
...
var LatexSuitePlugin = class extends import_obsidian5.Plugin {
  constructor() {
    super(...arguments);
      ...
      for (const tr of update.transactions) {
        for (const effect4 of tr.effects) {
          if (effect4.is(startSnippet)) {
            if (redoTr) {
              (0, import_commands3.redo)(update.view);
              (0, import_commands3.redo)(update.view);
              (0, import_commands3.redo)(update.view);
            }
          } else if (effect4.is(undidEndSnippet)) {
            if (undoTr) {
              (0, import_commands3.undo)(update.view);
              (0, import_commands3.undo)(update.view);
              (0, import_commands3.undo)(update.view);
            }
          }
    ...
    this.getInvertedEffects = () => {
      return import_commands3.invertedEffects.of((tr) => {
        const effects = [];
        for (const effect4 of tr.effects) {
          if (effect4.is(addMark)) {
    ...
    this.autoEnlargeBrackets = (view) => {
      ...
      for (let i = start2; i < end2; i++) {
        ...
        if (!(is1CharBracket || isCurlyBracket))
          continue;
  ...
  onload() {
    return __async(this, null, function* () {
      ...
      if ((_a = this.app.vault.config) == null ? void 0 : _a.legacyEditor) {
        ...
        new import_obsidian5.Notice(message, 1e5);
        console.log(message);
        return;
      }
  ...
  onFileChange(file) {
    return __async(this, null, function* () {
      ...
      if (file.path === this.settings.snippetsFileLocation) {
        try {
          yield this.setSnippetsFromFile(file.path);
          new import_obsidian5.Notice("Successfully reloaded snippets.", 5e3);
        } catch (e) {
          new import_obsidian5.Notice("Failed to load snippets: there are syntax errors in the file " + +this.settings.snippetsFileLocation, 5e3);
        }
      }
    });
  }
